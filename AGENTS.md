...
**The loop is bounded, and patching is not the only exit.** Iterative revision explores the neighborhood of the first attempt: it converges on a local optimum, and past a couple of rounds it starts sanding away voice instead of improving the work. After two re-assessments of the same artifact, never run a third full patch round. Exit one of three ways:

- **Approve with notes** — the residual findings are minor; they become Concerns in the report.
- **Regenerate** — throw the artifact away and produce it fresh: a new attempt in a fresh context with the accumulated findings attached as constraints, entering assessment as round one of a new bounded loop. Two failed revision rounds is strong evidence the problem is the *skeleton*, not the flesh — and more patching of a wrong skeleton is the least effective possible work. For **planning artifacts (Steps 2–5), regeneration is the default cap-hit exit**: a scene shipped with notes costs one mediocre scene; a book plot shipped with notes costs the book. An Assessor may recommend regeneration at any round when it judges the problems pervasive rather than local. One regeneration cycle, then the author decides.
- **Escalate to the author** — the remaining issues are creative decisions (which character carries a theme, which book a thread resolves in). Present them plainly with your recommendation. This is also the exit when findings indict the *plan* an artifact was built from — regeneration can't fix a plan problem.
...
**Exception — mechanical verification.** The continuity, copy, and proofreading passes (6.5–6.7) check against explicit, closed criteria (spelling, grammar, ledger facts). Each runs as a single independent pass that judges *and* fixes — no findings-only round-trip, because a separate generative role buys nothing when the correction is mechanical. "Mechanical" names the verify-and-fix *protocol*, not a model tier: their bookkeeping is laced with judgment (dialogue-meaning preservation, voice-preserving corrections, snag detection), so independence means a fresh context on a **strong** model, per each pass doc's tier line.
...
Quality is the project's sole optimization target; model choice follows from it. Tiers are defined by capability, never vendor or model name, so the workflow stays portable.

- **Craft tier — the strongest available model.** Everything that generates, judges, or *reads* prose and story — which is nearly everything: first draft (6.1), developmental assessment (6.2), the Cold Read at every scope, voice rewrite (6.3) and its candidate judges, line assessment (6.4), **continuity check (6.5), copy edit (6.6), and proofread (6.7)** — each of those three mixes judgment into its bookkeeping (semantic preservation, voice-preserving corrections, snag detection), and the last reads a book ever gets are no place for a weak model — plus all plotting and interview steps and the Assessor running any rubric.
- **Mechanical tier — a fast, inexpensive model, optional.** True bookkeeping with no judgment content and no prose contact: ledger updates, word counts, exports, state-detection scans, dialogue-line extraction for the act audit. Nothing that touches manuscript prose ever runs here.

When a pass mixes judgment and mechanics, it is craft tier. When in doubt, craft tier — this project optimizes for quality, not spend. Where the workflow calls for a *different* model (the Voice Rewrite, decorrelated judges), that means a different **strong** model, never a weaker one: the value is in breaking shared tics, not in saving compute. Each `.proselon/workflow/` doc declares its tier near the top. Without per-task model selection, the declarations are inert and everything runs on the session model.
